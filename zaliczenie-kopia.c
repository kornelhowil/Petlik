...
/* Funkcje kompilatora */
ins *compiler(char *petlik, int length);
bool optimal(int *i, char *petlik, int length);
int compile(int *i, int j, char *petlik, ins *m_code, int length);
...
void interpreter(ins *m_code, var *variables);
/* Główny program */
int main(void)
{	
	var *variables = init();
	...
	while((c = getchar()) != EOF) {
	    ...
	    if (petlik[0] != '=') {
	    	ins *m_code = compiler(petlik, length);
...
ins *compiler(char *petlik, int length)
{
	ins *m_code = malloc((size_t)(length + 1) * sizeof(ins));
	int index = 0;
